...
# alef:hash:b64770603465cdb3421593c2f12b7508345a9a3441b95b79ad5a8be9f974c3d1
...
defmodule E2e.EdgeCasesTest do
  ...
  describe "visitor_custom_element_with_nesting" do
    test "Visitor handles custom elements with nested content" do
      ...
      {:ok, result} = HtmlToMarkdown.convert("<div><custom-widget data-value=\"123\"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>", visitor)
  ...
  describe "visitor_deeply_nested_skip" do
    test "Visitor skips deeply nested elements" do
      ...
      {:ok, result} = HtmlToMarkdown.convert("<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>", visitor)
  ...
  describe "visitor_element_end_modification" do
    test "Visitor modifies element at end after children processed" do
      visitor = %{
      :handle_element_end => fn(_ctx, _output) ->
        {:custom, "MODIFIED OUTPUT"}
      end,
    }

      {:ok, result} = HtmlToMarkdown.convert("<blockquote><p>Original quote</p></blockquote>", visitor)
      assert result.content != ""
    end
  end
  ...
  describe "visitor_element_start_skip_entire_subtree" do
    test "Visitor skips at element_start level removes entire subtree" do
      visitor = %{
      :handle_element_start => fn(_ctx) ->
        :skip
      end,
    }

      {:ok, result} = HtmlToMarkdown.convert("<div><h1>Title</h1><p>Content</p></div>", visitor)
      assert result.content != ""
    end
  end
  ...
  describe "visitor_unknown_tag_preservation" do
    test "Visitor preserves unknown HTML tags as raw HTML" do
      ...
      {:ok, result} = HtmlToMarkdown.convert("<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>", visitor)
