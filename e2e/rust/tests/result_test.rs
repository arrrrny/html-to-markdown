...
// alef:hash:b9bae898deba068fcc4eb86cd3ed0884db8c7f5d354ef0b845edae55e3cc918f
...
fn test_result_tables_empty_when_no_tables() {
    ...
    assert_eq!(result.tables.len(), 0, "expected exactly 0 elements, got {}", result.tables.len());
}
...
fn test_result_tables_multiple() {
    ...
    let options = Some(serde_json::from_value(options_json).unwrap());
    let result = convert(&html, &options).expect("should succeed");
    assert!(result.tables.len() >= 2, "expected at least 2 elements, got {}", result.tables.len());
}
...
fn test_result_warnings_empty_for_clean_input() {
    ...
    let result = convert(&html, &options).expect("should succeed");
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(result.warnings.len(), 0, "expected exactly 0 elements, got {}", result.warnings.len());
}
