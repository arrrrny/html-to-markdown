...
// alef:hash:c3ae9690801d292552de4469d660363e9374b92c31a0273b68f95ef0f3974a88
...
fn test_visitor_custom_element_with_nesting() {
    ...
    let content = result.content.as_deref().unwrap_or("");
    assert!(format!("{:?}", content).contains(r#"[CUSTOM WIDGET]"#), "expected to contain: {}", r#"[CUSTOM WIDGET]"#);
    assert!(!format!("{:?}", content).contains(r#"Widget content here"#), "expected NOT to contain: {}", r#"Widget content here"#);
}
...
fn test_visitor_deeply_nested_skip() {
    ...
    assert!(!format!("{:?}", content).contains(r#"highlight"#), "expected NOT to contain: {}", r#"highlight"#);
}
...
fn test_visitor_element_end_modification() {
    ...
    struct _TestVisitor;
    impl HtmlVisitor for _TestVisitor {
        fn visit_element_end(&self, ctx, output) -> VisitResult {
...
fn test_visitor_element_start_skip_entire_subtree() {
    ...
    struct _TestVisitor;
    impl HtmlVisitor for _TestVisitor {
        fn visit_element_start(&self, ctx) -> VisitResult {
            VisitResult::Skip
        }
    }
    let visitor = std::rc::Rc::new(std::cell::RefCell::new(_TestVisitor));
...
fn test_visitor_unknown_tag_preservation() {
    ...
    assert!(format!("{:?}", content).contains(r#"More article text"#), "expected to contain: {}", r#"More article text"#);
    assert!(format!("{:?}", content).contains(r#"<x-custom>"#), "expected to contain: {}", r#"<x-custom>"#);
}
