// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:0aacc989ae976f0e2ce28f6d6ba07c1588ba99fc0d1399aae63ef93356613045
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef

// E2e tests for category: result
package e2e_test

import (
	"testing"

	"github.com/stretchr/testify/assert"

	htmd "github.com/kreuzberg-dev/html-to-markdown/packages/go/v3"
)

func Test_ResultTablesEmptyWhenNoTables(t *testing.T) {
	// Result tables array is empty when input has no tables
	result, err := htmd.Convert(`<p>No tables here</p>`, htmd.NewConversionOptions(htmd.WithConversionOptionsIncludeDocumentStructure(true)))
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.Equal(t, len(result.Tables), 0, "expected exactly 0 elements")
}

func Test_ResultTablesMultiple(t *testing.T) {
	// Multiple tables each appear in the tables array
	result, err := htmd.Convert(`<table><tr><th>A</th></tr><tr><td>1</td></tr></table><p>Between</p><table><tr><th>B</th></tr><tr><td>2</td></tr></table>`, htmd.NewConversionOptions(htmd.WithConversionOptionsIncludeDocumentStructure(true)))
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	assert.GreaterOrEqual(t, len(result.Tables), 2, "expected at least 2 elements")
}

func Test_ResultTablesSimple(t *testing.T) {
	// Simple table populates the tables array in result
	result, err := htmd.Convert(`<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>`, htmd.NewConversionOptions(htmd.WithConversionOptionsIncludeDocumentStructure(true)))
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.GreaterOrEqual(t, len(result.Tables), 1, "expected at least 1 elements")
}

func Test_ResultTablesWithoutStructureFlag(t *testing.T) {
	// Tables array is empty when includeDocumentStructure is false
	result, err := htmd.Convert(`<table><tr><th>X</th></tr><tr><td>Y</td></tr></table>`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.Equal(t, len(result.Tables), 0, "expected exactly 0 elements")
}

func Test_ResultWarningsEmptyForCleanInput(t *testing.T) {
	// Warnings array is empty for well-formed HTML without problematic content
	result, err := htmd.Convert(`<h1>Title</h1><p>Clean content with <a href='https://example.com'>a link</a>.</p>`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.Equal(t, len(result.Warnings), 0, "expected exactly 0 elements")
}

func Test_ResultWarningsEmptyForComplexInput(t *testing.T) {
	// Warnings array is empty for complex but valid HTML
	result, err := htmd.Convert(`<article><h1>Article</h1><p>Paragraph with <strong>bold</strong> and <em>italic</em>.</p><table><tr><th>Col</th></tr><tr><td>Val</td></tr></table><ul><li>Item 1</li><li>Item 2</li></ul></article>`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.Equal(t, len(result.Warnings), 0, "expected exactly 0 elements")
}

func Test_ResultWarningsEmptyForMalformedHtml(t *testing.T) {
	// Warnings array is empty even for malformed HTML (parser is lenient)
	result, err := htmd.Convert(`<p>Unclosed paragraph<div>Mixed nesting</p></div>`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil || len(*result.Content) == 0 {
		t.Errorf("expected non-empty value")
	}
	assert.Equal(t, len(result.Warnings), 0, "expected exactly 0 elements")
}
