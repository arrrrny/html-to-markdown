...
// alef:hash:68f464c4b3c632c7868da8f863f7af25aec27a3d01543046f1ca5822e504ad72
...
describe('result', () => {
  it('result_tables_empty_when_no_tables: Result tables array is empty when input has no tables', () => {
    const result = convert("<p>No tables here</p>", { includeDocumentStructure: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
    expect(result.tables.length).toBe(0);
  });

  it('result_tables_multiple: Multiple tables each appear in the tables array', () => {
    const result = convert("<table><tr><th>A</th></tr><tr><td>1</td></tr></table><p>Between</p><table><tr><th>B</th></tr><tr><td>2</td></tr></table>", { includeDocumentStructure: true });
  ...
  it('result_tables_simple: Simple table populates the tables array in result', () => {
    const result = convert("<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>", { includeDocumentStructure: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
