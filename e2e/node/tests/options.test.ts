...
// alef:hash:79e611bceb03984970ba17dd189b359acec60019f6debc25119a430a9d41875b
...
describe('options', () => {
  it('options_autolinks_false: Bare URL links rendered as regular markdown links when autolinks disabled', () => {
    const result = convert("<p><a href='https://example.com'>https://example.com</a></p>", { autolinks: false });
    expect(result.content).toContain("example.com");
  });

  it('options_br_in_tables_false: BR elements in table cells are stripped when disabled', () => {
    const result = convert("<table><tr><th>Col</th></tr><tr><td>A<br>B</td></tr></table>", { brInTables: false });
    expect(result.content).toContain("Col");
  });

  it('options_br_in_tables_true: BR elements in table cells render as line breaks', () => {
    const result = convert("<table><tr><th>Header</th></tr><tr><td>Line 1<br>Line 2</td></tr></table>", { brInTables: true });
  ...
  it('options_code_block_backticks: Backticks code block style uses triple backtick fences', () => {
    const result = convert("<pre><code class=\"language-js\">console.log('hi');</code></pre>", { codeBlockStyle: "Backticks" });
    expect(result.content).toContain("```");
    expect(result.content).toContain("console.log('hi');");
  });

  it('options_code_block_indented: Code blocks use 4-space indentation', () => {
    const result = convert("<pre><code>print('hello')</code></pre>", { codeBlockStyle: "Indented" });
    expect(result.content).toContain("print('hello')");
    expect(result.content).not.toContain("```");
  });

  it('options_code_block_tildes: Code blocks use tilde fences', () => {
    const result = convert("<pre><code>let x = 1;</code></pre>", { codeBlockStyle: "Tildes" });
    expect(result.content).toContain("~~~");
  ...
  it('options_code_block_tildes_style: Tildes code block style uses triple tilde fences', () => {
    const result = convert("<pre><code>some code</code></pre>", { codeBlockStyle: "Tildes" });
    expect(result.content).toContain("~~~");
  ...
  it('options_escape_asterisks: escape_asterisks option escapes asterisks in plain text', () => {
    const result = convert("<p>Use 2*3 = 6 in math.</p>", { escapeAsterisks: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_escape_misc: escape_misc option escapes miscellaneous markdown characters', () => {
    const result = convert("<p>Use # and | and ~ in text.</p>", { escapeMisc: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_escape_underscores: escape_underscores option escapes underscores in plain text', () => {
    const result = convert("<p>The variable_name is defined.</p>", { escapeUnderscores: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_exclude_selectors_attribute: Elements matching CSS attribute selector are excluded entirely', () => {
    const result = convert("<body><div role=\"complementary\">Sidebar</div><p>Primary text</p></body>", { excludeSelectors: ["[role='complementary']"] });
  ...
  it('options_exclude_selectors_class: Elements matching CSS class selector are excluded entirely', () => {
    const result = convert("<body><div class=\"cookie-banner\">Accept cookies</div><p>Main content</p></body>", { excludeSelectors: [".cookie-banner"] });
  ...
  it('options_exclude_selectors_empty_noop: Empty exclude_selectors list does not affect output', () => {
    const result = convert("<p>Hello world</p>", { excludeSelectors: [] });
    expect(result.content ?? "").toContain("Hello world");
  });

  it('options_exclude_selectors_id: Elements matching CSS id selector are excluded entirely', () => {
    const result = convert("<body><div id=\"ad-container\">Buy stuff</div><p>Article text</p></body>", { excludeSelectors: ["#ad-container"] });
  ...
  it('options_exclude_selectors_multiple: Multiple CSS selectors each exclude their matched elements', () => {
    const result = convert("<body><nav class=\"nav\">Menu</nav><p>Content</p><footer>Footer</footer></body>", { excludeSelectors: [".nav", "footer"] });
  ...
  it('options_exclude_selectors_nested_content_dropped: All descendants of excluded elements are dropped', () => {
    const result = convert("<body><aside class=\"sidebar\"><h2>Related</h2><p>Sidebar text</p></aside><main><p>Main text</p></main></body>", { excludeSelectors: [".sidebar"] });
  ...
  it('options_exclude_selectors_plain_text_mode: Exclude selectors work in plain text output mode', () => {
    const result = convert("<body><div class=\"nav\">Navigation</div><p>Article body</p></body>", { excludeSelectors: [".nav"], outputFormat: "Plain" });
  ...
  it('options_exclude_selectors_vs_strip_tags: exclude_selectors drops entire subtree unlike strip_tags which keeps children', () => {
    const result = convert("<body><div class=\"wrapper\"><p>Inner paragraph</p></div><p>Outer text</p></body>", { excludeSelectors: [".wrapper"] });
  ...
  it('options_extract_metadata_true: Extract metadata returns document metadata when enabled', () => {
    const result = convert("<html><head><title>Test Page</title><meta name='description' content='A test page'></head><body><p>Content</p></body></html>", { extractMetadata: true });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_heading_style_atx: ATX heading style produces hash-prefixed headings', () => {
    const result = convert("<h1>Title</h1><h2>Subtitle</h2>", { headingStyle: "Atx" });
  ...
  it('options_heading_style_atx_closed: ATX closed heading style adds closing hashes', () => {
    const result = convert("<h1>Closed Heading</h1>", { headingStyle: "AtxClosed" });
  ...
  it('options_list_custom_bullets: Custom bullet character for unordered lists', () => {
    const result = convert("<ul><li>Item A</li><li>Item B</li></ul>", { bullets: "*" });
  ...
  it('options_list_indent_tabs: Tab indentation type for nested list items', () => {
    const result = convert("<ul><li>Parent<ul><li>Child</li></ul></li></ul>", { listIndentType: "Tabs" });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_list_indent_width_four: Nested lists indented with 4 spaces per level', () => {
    const result = convert("<ul><li>Outer<ul><li>Inner</li></ul></li></ul>", { listIndentWidth: 4 });
  ...
  it('options_max_depth_truncates: max_depth truncates content beyond the specified depth', () => {
    const result = convert("<div><p>Shallow</p><div><div><div><p>Too deep</p></div></div></div></div>", { maxDepth: 3 });
  ...
  it('options_output_format_markdown: Default markdown output format produces standard markdown', () => {
    const result = convert("<h1>Title</h1><p>Some text.</p>", { headingStyle: "Atx", outputFormat: "Markdown" });
    expect(result.content).toContain("# Title");
    expect(result.content).toContain("Some text.");
  });

  it('options_output_format_plain: Plain text output format strips markdown syntax', () => {
    const result = convert("<h1>Title</h1><p>Some <strong>bold</strong> text.</p>", { outputFormat: "Plain" });
    expect(result.content).toContain("Title");
  ...
  it('options_preprocessing_aggressive: Aggressive preset removes nav, footer, aside unconditionally', () => {
    const result = convert("<nav>Menu</nav><article><h1>Title</h1><p>Content</p></article><aside>Sidebar</aside><footer>Footer</footer>", { preprocessing: { preset: "Aggressive" } });
    expect(result.content).toContain("Title");
    expect(result.content).toContain("Content");
    expect(result.content).not.toContain("Menu");
  });
  ...
  it('options_preprocessing_minimal: Minimal preset preserves nav, footer, aside', () => {
    const result = convert("<nav>Navigation</nav><p>Content</p><footer>Footer</footer>", { preprocessing: { preset: "Minimal" } });
  ...
  it('options_preprocessing_remove_forms: Forms are removed when remove_forms is true', () => {
    const result = convert("<p>Before</p><form><input type='text'/><button>Submit</button></form><p>After</p>", { preprocessing: { removeForms: true } });
  ...
  it('options_preserve_tags_iframe: Iframe tags preserved as raw HTML in output', () => {
    const result = convert("<p>Before</p><iframe src='video.html' width='560'></iframe><p>After</p>", { preserveTags: ["iframe"] });
    expect(result.content).toContain("Before");
    expect(result.content).toContain("After");
  ...
  it('options_skip_images_true: Images are omitted from output when skipImages is true', () => {
    const result = convert("<p>Before <img src='test.jpg' alt='photo'> After</p>", { skipImages: true });
    expect(result.content).toContain("Before");
    expect(result.content).toContain("After");
  ...
  it('options_strip_newlines: Strip newlines produces single-line paragraphs', () => {
    const result = convert("<p>First paragraph.</p><p>Second paragraph.</p>", { stripNewlines: true });
    expect(result.content).toContain("First paragraph.");
    expect(result.content).toContain("Second paragraph.");
  });

  it('options_strip_tags_div_span: Div and span tags stripped but content preserved', () => {
    const result = convert("<div class='wrapper'><p>Inside div</p></div><p>Outside <span class='hl'>span text</span></p>", { stripTags: ["div", "span"] });
  ...
  it('options_strong_em_underscore: Strong and em tags use underscore symbol instead of asterisk', () => {
    const result = convert("<p><strong>bold</strong> and <em>italic</em></p>", { strongEmSymbol: "_" });
  ...
  it('options_sub_symbol_tilde: Subscript rendered with tilde symbol', () => {
    const result = convert("<p>H<sub>2</sub>O</p>", { subSymbol: "~" });
    expect(result.content).toContain("~2~");
  });

  it('options_sup_symbol_caret: Superscript rendered with caret symbol', () => {
    const result = convert("<p>x<sup>2</sup></p>", { supSymbol: "^" });
    expect(result.content).toContain("^2^");
  });

  it('options_whitespace_normalized: Normalized whitespace mode collapses multiple spaces', () => {
    const result = convert("<p>Text   with    extra   spaces.</p>", { whitespaceMode: "Normalized" });
    expect((result.content ?? "").length).toBeGreaterThan(0);
    expect(result.content).toContain("Text");
  ...
  it('options_whitespace_strict: Strict whitespace mode preserves whitespace as-is', () => {
    const result = convert("<p>Preserved   spacing.</p>", { whitespaceMode: "Strict" });
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it('options_wrap_disabled: Wrap option disabled preserves long lines without breaking', () => {
    const result = convert("<p>This is a long paragraph that should not be wrapped at all because wrapping is disabled.</p>", { wrap: false });
  ...
  it('options_wrap_enabled: Wrap option enabled with custom width wraps long lines', () => {
    const result = convert("<p>This is a long paragraph that should be wrapped at the specified column width when the wrap option is enabled.</p>", { wrap: true, wrapWidth: 40 });
    expect((result.content ?? "").length).toBeGreaterThan(0);
