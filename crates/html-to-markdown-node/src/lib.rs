...
// alef:hash:5412735c00a62641d94599d3f01f28dc84605e1210510da09c276182a2020f68
...
#[napi]
pub fn convert(html: String, options: JsConversionOptions) -> Result<JsConversionResult> {
    let visitor_handle = options.visitor.clone().map(|v| {
        ...
        match result {
            ...
            Ok(val) => {
                    ...
                    .and_then(|s| s.into_owned())
                {
                    ...
                    if let Ok(custom_val) = obj.get_named_property::<napi::bindgen_prelude::Unknown>("custom") {
                        if let Ok(s) = custom_val
                            .coerce_to_string()
                            .and_then(|s| s.into_utf8())
                            .and_then(|s| s.into_owned())
                        {
                    ...
                    } else if let Ok(error_val) = obj.get_named_property::<napi::bindgen_prelude::Unknown>("error") {
                        if let Ok(s) = error_val
                            .coerce_to_string()
                            .and_then(|s| s.into_utf8())
                            .and_then(|s| s.into_owned())
                        {
                            html_to_markdown_rs::VisitResult::Error(s)
                        } else {
                            html_to_markdown_rs::VisitResult::Continue
                        }
                    } else {
